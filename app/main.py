...
async def create_reviewer(reviewer: schemas.ReviewerCreate, db: AsyncSession = Depends(get_db)):
...
async def read_reviewers(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
    reviewers = await crud.get_reviewers(db, skip=skip, limit=limit)
    return reviewers

@app.put("/reviewers/{reviewer_id}", response_model=schemas.Reviewer)
async def update_reviewer(reviewer_id: int, reviewer: schemas.ReviewerCreate, db: AsyncSession = Depends(get_db)):
    db_reviewer = await crud.update_reviewer(db, reviewer_id=reviewer_id, reviewer_update=reviewer)
    if db_reviewer is None:
        raise HTTPException(status_code=404, detail="Reviewer not found")
    return db_reviewer
...
async def update_review(review_id: int, review: schemas.ReviewCreate, db: AsyncSession = Depends(get_db)):
